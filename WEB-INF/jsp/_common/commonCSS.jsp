...
<link rel="bookmark" href="${ctx}/assets/favicon.ico">
...
<!-- page plugin css -->
<link rel="stylesheet" href="${ctx}/assets/components/artDialog/css/ui-dialog.css"><!--artDialog 6.x-->
...
<!-- 思源黑体 -->
<link rel="stylesheet" href="${ctx}/assets/blue/fonts/SourceHanSansCN-Bold.css">
<link rel="stylesheet" href="${ctx}/assets/blue/fonts/SourceHanSansCN-Normal.css">
...
<script type="text/javascript">
    ...
    //皮肤初始化
    $(function(){
        var thsSkin = "<%=String.valueOf(ths.jdp.core.context.PropertyConfigure.getProperty("jdp.app.defaultskin"))%>";
        if(thsSkin!=""){
            if(!$(window.document.body).hasClass("login-layout")){
                $(window.document.body).attr("class", thsSkin);
            }
        }
        var breadcrumbsLevel = "<%=ths.jdp.util.Tool.isNull(String.valueOf(ths.jdp.core.context.PropertyConfigure.getProperty("jdp.breadcrumbs.level"))) ? false :true %>";

    	 // MIS系统面包屑
        if (breadcrumbsLevel === "true"){
        	// 发送消息给顶层窗口
        	window.top.postMessage('{"messageType":"askBreadcrumbs","resId": "${THS_JDP_RES_ID}"}','*');
        }

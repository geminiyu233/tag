...
<script type="text/javascript">
	...
	function dealUrl(menu){
		...
		if ('${tomenuid}') url += "&tomenuid=${tomenuid}";
		if ('${requestTopFloor}') url += "&requestTopFloor=${requestTopFloor}";
	...
	function editPassword(){
		...
		var ouContext = "<%=ths.jdp.core.context.PropertyConfigure.getProperty("jdp.ou.api.context").toString()%>";
		var url = ouContext+'/password/edit?loginName='+loginName;
		window.open(url);	// 新页面打开
	}
	//写访问日志
	function writeAccessLog(o){
		...
		$.ajax({
			...
			data : {"OPERATION":$(o).find(".menu-text").text()},
			cache : false,
			success : function(leftMenuHtml) {
